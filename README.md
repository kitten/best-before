...
Origin revalidation normally strips conditional headers and fetches a complete `200` response. Set `conditionalRevalidation: true` to send the stored validators when revalidating a stale entry. This is an opt-in efficiency optimization. When the origin returns `304 Not Modified`, the cache reuses the stored body and refreshes its metadata.

If a 304 includes a new cache policy, that policy must still allow the response to be stored. Directives omitted from the new policy are generally not carried over, so the cache removes the entry when the new policy is not storable. One exception applies to an entry already admitted to shared storage: if the 304 echoes bare `no-cache`, the entry remains eligible unless the 304 also specifies `no-store` or `private`.

Responses carrying `no-cache` are handled differently from ordinary stale entries. When an otherwise storable response has an `ETag` or `Last-Modified` validator, the cache retains it for at least one day, or for a longer explicitly configured lifetime. This retention period only keeps the response available for validation; it does not make the response fresh. The cache contacts the origin before every reuse and sends the validators even when `conditionalRevalidation` is disabled. Responses without validators are not retained.

In a shared cache, `no-cache` also requires `public`, `s-maxage`, `must-revalidate`, `proxy-revalidate`, or a CDN-specific cache policy. `immutable` and stale-serving extensions are not sufficient on their own. Bare shared `no-cache` is not retained, regardless of `requireSharedDirective`. A `no-cache` response to an authorized request is retained only when one of the qualifying directives allows shared storage. Private caches may retain bare validator-backed `no-cache`. Responses carrying `no-store` are never stored.
