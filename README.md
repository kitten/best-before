...
However, origin revalidation defaults to re-fetching a full 200 response by stripping conditional headers. Set `conditionalRevalidation: true` to instead send the stored entry's validator headers when revalidating a stale entry. On a 304, the retained body is re-served and re-stored with refreshed metadata. If the 304 supplies a cache policy, that updated policy must itself remain eligible for storage; admission directives omitted by the update are not inherited, and the entry is removed when the new policy is not storable. This is normally an opt-in efficiency optimization.

A response carrying `no-cache` is the exception: when it has an `ETag` or `Last-Modified` validator and is otherwise storable, it is retained for at least one day (or for a longer explicitly configured lifetime). This is a store-facing retention period, not a freshness lifetime: the response is never reused without contacting the origin, and its validators are sent even when `conditionalRevalidation` is disabled. Validator-less `no-cache` responses are not retained.

In a shared cache, `no-cache` also requires an explicit shared-cache signal: `public`, `s-maxage`, `must-revalidate`, `proxy-revalidate`, or a CDN-specific cache policy. `immutable` and stale-serving extensions do not establish shared eligibility on their own. Bare shared `no-cache` is not retained, regardless of `requireSharedDirective`, and neither is a `no-cache` response to a request carrying `Authorization` unless one of the qualifying signals permits it. A private cache may retain bare validator-backed `no-cache`. `no-store` responses are never stored.
