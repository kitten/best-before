...
In a shared cache, `no-cache` also requires an explicit shared-cache signal such as `public`, `s-maxage`, `must-revalidate`, `proxy-revalidate`, `immutable`, or a CDN-specific cache policy. Bare shared `no-cache` is not retained, regardless of `requireSharedDirective`. A private cache may retain bare validator-backed `no-cache`. `no-store` responses are never stored.
